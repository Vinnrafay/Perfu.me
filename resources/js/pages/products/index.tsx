...
export default function Catalog({ products = [] }: Props) {
    ...
    return (
        ...
        <div className="min-h-screen bg-white text-neutral-900 font-sans pb-20">
            ...
            <section className="bg-neutral-950 text-white py-24 px-6 flex flex-col items-center text-center">
            ...
            {/* Main Content Layout */}
            <div className="container mx-auto px-4 md:px-8 pt-12 flex flex-col md:flex-row gap-10">
                    ...
                    <div className="sticky top-24">
                        ...
                        <Accordion type="multiple" defaultValue={['kategori', 'gender', 'harga']} className="w-full">
                            ...
                            <AccordionItem value="kategori" className="border-b-0 mb-2">
                                <AccordionTrigger className="hover:no-underline text-sm font-medium uppercase tracking-wider text-neutral-500">
                                    Kategori
                                </AccordionTrigger>
                ...
                <main className="flex-1">
                    ...
                    {filteredProducts.length === 0 ? (
                        <div className="h-64 flex items-center justify-center border border-dashed border-neutral-200 rounded-lg">
                            <p className="text-neutral-400">Tidak ada produk yang sesuai dengan filter.</p>
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-12">
                            {filteredProducts.map((product) => (
                                // Card flat & clean tanpa shadow bawaan
                                <Card key={product.id} className="border-0 shadow-none rounded-none group cursor-pointer bg-transparent">
                                    
                                    {/* Image Area */}
                                    <CardContent className="p-0 relative overflow-hidden aspect-[3/4] bg-neutral-100 flex items-center justify-center mb-4">
                                        {product.Foto ? (
                                            <img 
                                            ...
                                            <span className="text-neutral-300 font-medium">No Image</span>
                                        )}
                                        
                                        {/* Floating Badges */}
                                        <div className="absolute top-3 left-3 flex flex-col gap-2 items-start">
                                            {product['Best Seller'] === 'yes' && (
                                                <Badge className="bg-neutral-950 hover:bg-neutral-950 text-white rounded-sm px-2 py-0.5 text-[10px] font-semibold uppercase tracking-widest">
                                                    Best Seller
                                                </Badge>
                                            )}
                                            <Badge variant="secondary" className="bg-white/90 text-neutral-900 hover:bg-white rounded-sm px-2 py-0.5 text-[10px] uppercase font-medium">
                                                {product.kategori}
                                            </Badge>
                                        </div>
                                        ...
                                        <div className="absolute bottom-4 left-4 right-4 opacity-0 translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300">
                                            <Button className="w-full bg-white/90 backdrop-blur text-neutral-950 hover:bg-neutral-950 hover:text-white rounded-none h-11">
                                                Lihat Detail
                                            </Button>
                                        </div>
                                    </CardContent>
                                    ...
                                    <CardFooter className="p-0 flex flex-col items-start gap-1">
                                        <div className="w-full flex justify-between items-start">
                                            <h3 className="font-semibold text-base text-neutral-900 uppercase tracking-tight">
                                        ...
                                        <p className="text-sm text-neutral-500 capitalize">{product.Varian}</p>
